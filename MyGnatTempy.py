...
def test_upsample_block():
    ...
    in_channels = 64
    out_channels = 32
    skip_channels = 32  # Add this parameter
    height = 32
    width = 32
    embed_dim = 128
    ...
    input_tensor = torch.randn(batch_size, in_channels, height, width)
    skip_tensor = torch.randn(batch_size, skip_channels, height * 2, width * 2)  # Match the skip_channels
    embed_vector = torch.randn(batch_size, embed_dim)

    # Instantiate the Upsample Block
    upsample_block = UpsampleBlock(in_channels, out_channels, embed_dim, skip_channels)
...
def test_unet():
    ...
    width = 64
    embed_dim = 128
    timesteps = 1000  # Ensure it matches the model configuration

    # Create random input tensors
    input_tensor = torch.randn(batch_size, input_channels, height, width)

    # Create timestep embeddings
    t = torch.randint(0, timesteps, (batch_size,))  # Random timesteps for each batch
    embed_vector = torch.nn.functional.one_hot(t, num_classes=timesteps).float()

    # Instantiate the U-Net
    unet_model = UNet(input_channels, output_channels, embed_dim, timesteps)
...
class LightweightUNet(UNet):
    def __init__(self, input_channels, output_channels, embed_dim, timesteps):
        ...
        self.down1 = DownsampleBlock(64, 64, embed_dim)  # Input: 64, Output: 64
        self.down2 = DownsampleBlock(64, 128, embed_dim)  # Input: 64, Output: 128
        self.down3 = DownsampleBlock(128, 256, embed_dim)  # Input: 128, Output: 256
        self.up3 = UpsampleBlock(256, 128, embed_dim, skip_channels=128)  # Skip: 128
        self.up2 = UpsampleBlock(128, 64, embed_dim, skip_channels=64)  # Skip: 64
        self.up1 = UpsampleBlock(64, 64, embed_dim, skip_channels=64)  # Skip: 64
        self.final_conv = nn.Conv2d(64, output_channels, kernel_size=1)  # Output channels: 3
...
#HAVE TO RUN THIS COMMAND TO GET THE CODE TO WORK:
'''
